...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, groups=8, block_size=41, graph=None, graph_args=dict(), in_channels=3):
        ...
        A = np.array(self.graph.A)
        # logger.info(A)
        self.data_bn = nn.BatchNorm1d(num_person * in_channels * num_point)
        self.bn_init(self.data_bn, 1)
    ...
    def feature_extractor(self, x, keep_prob=0.9):
        N, C, T, V, M = x.size()
    ...
    def forward(self, x, keep_prob=0.9):
        x = self.feature_extractor(x, keep_prob)
